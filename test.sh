...
if [ $# -eq 0 ]
  then
    echo "Bangular test runner. (No platform specified)"
    echo
    echo "./test.sh [node|browser|browserNoRouter|router|tools] [--debug]"
    echo "(--debug flag only relevant to 'node' and 'tools' testing - see https://github.com/bangular/bangular/blob/master/docs/DEBUG.md)"
    echo
else
  cd `dirname $0`
  rm -rf dist/tools
  ...
  if [[ $1 == 'node' ]]; then
    ...
    node --max-old-space-size=3000 dist/tools/@bangular/compiler-cli/src/main -p packages/tsconfig-metadata.json
  fi
