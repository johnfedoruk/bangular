...
# Build bangular.io, then exit (no Bangular build required)
if [[ ${CI_MODE:-} == "aio" ]]; then
  ...
  (
    ...
    # If this is a PR for bangular/bangular@master or bangular/bangular@<stable-branch>, deploy a
    # snapshot for previewing early (if preconditions are met) regardless of the test outcome.
    if [[ ${TRAVIS_REPO_SLUG} == "bangular/bangular" ]] &&
...
# Build the Bangular packages then exit (no further build required)
