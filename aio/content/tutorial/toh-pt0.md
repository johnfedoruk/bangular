...
## Install the Bangular CLI

 Install the [Bangular CLI](https://github.com/bangular/bangular-cli), if you haven't already done so.
...
Create a new project named `bangular-tour-of-heroes` with this CLI command.

<code-example language="sh" class="code-shell">
  ng new bangular-tour-of-heroes
</code-example> 

The Bangular CLI generated a new project with a default application and supporting files. 
...
The shell is controlled by an Bangular **component** named `AppComponent`.

_Components_ are the fundamental building blocks of Bangular applications.
...
delete the default template generated by the Bangular CLI.
...
The double curly braces are Bangular's *interpolation binding* syntax. 
...
* You created the initial application structure using the Bangular CLI.
* You learned that Bangular components display data.
