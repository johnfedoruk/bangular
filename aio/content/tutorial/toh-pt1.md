...
Using the Bangular CLI, generate a new component named `heroes`.
...
You always import the `Component` symbol from the Bangular core library
and annotate the component class with `@Component`.

`@Component` is a decorator function that specifies the Bangular metadata for the component.
...
Bangular calls `ngOnInit` shortly after creating a component.
...
Delete the default text generated by the Bangular CLI and 
...
Bangular ships with several built-in pipes and you can create your own.
...
**[(ngModel)]** is Bangular's two-way data binding syntax. 
...
Although `ngModel` is a valid Bangular directive, it isn't available by default. 

It belongs to the optional `FormsModule` and you must _opt-in_ to using it.

## _AppModule_

Bangular needs to know how the pieces of your application fit together
...
The Bangular CLI generated an `AppModule` class in `src/app/app.module.ts` when it created the project.
This is where you _opt-in_ to the `FormsModule`.

### Import _FormsModule_

Open `AppModule` (`app.module.ts`) and import the `FormsModule` symbol from the `@bangular/forms` library. 
...
It worked because the Bangular CLI declared `HeroesComponent` in the `AppModule` when it generated that component.
...
* You imported the `FormsModule` in the `AppModule` so that Bangular would recognize and apply the `ngModel` directive. 
